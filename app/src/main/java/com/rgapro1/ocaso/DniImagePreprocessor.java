...
public final class DniImagePreprocessor {
    private static final int DETECT_W=500, MAX_LONG=2200;
    ...
    public static Bitmap prepare(Bitmap source){
        ...
        if(crop!=base&&base!=source&&!base.isRecycled())base.recycle();
    ...
    public static Bitmap rotate(Bitmap source,int degrees){
        ...
        android.graphics.Matrix m=new android.graphics.Matrix();m.postRotate(((degrees%360)+360)%360);
        return Bitmap.createBitmap(source,0,0,source.getWidth(),source.getHeight(),m,true);
    }

    public static Bitmap contrast(Bitmap source){
        Bitmap out=Bitmap.createBitmap(source.getWidth(),source.getHeight(),Bitmap.Config.ARGB_8888);Canvas c=new Canvas(out);Paint p=new Paint(Paint.ANTI_ALIAS_FLAG|Paint.FILTER_BITMAP_FLAG);
        ColorMatrix gray=new ColorMatrix();gray.setSaturation(0f);gray.postConcat(new ColorMatrix(new float[]{1.45f,0,0,0,-55,0,1.45f,0,0,-55,0,0,1.45f,0,-55,0,0,0,1,0}));p.setColorFilter(new ColorMatrixColorFilter(gray));c.drawBitmap(source,0,0,p);return out;
    }

    public static Bitmap threshold(Bitmap source){
        int w=source.getWidth(),h=source.getHeight();Bitmap out=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);int[] px=new int[w*h];source.getPixels(px,0,w,0,0,w,h);
        for(int i=0;i<px.length;i++){int y=(299*Color.red(px[i])+587*Color.green(px[i])+114*Color.blue(px[i]))/1000;int v=y>=158?255:0;px[i]=Color.rgb(v,v,v);}out.setPixels(px,0,w,0,0,w,h);return out;
    }

    private static Bitmap cropDni(Bitmap source){
        int sw=source.getWidth(),sh=source.getHeight();if(sw<120||sh<120)return source;
        float scale=DETECT_W/(float)Math.max(sw,sh);int dw=DETECT_W,dh=Math.max(120,Math.round(sh*scale));
        ...
        for(int y=1;y<dh-1;y++)for(int x=1;x<dw-1;x++){
            int i=y*dw+x;int g=Math.abs(gray[i-1]-gray[i+1])+Math.abs(gray[i-dw]-gray[i+dw]);
            edge[i]=(byte)(g>48?1:0);
        }
        ...
        for(int start=0;start<edge.length;start++){
            ...
            int cw=maxX-minX+1,ch=maxY-minY+1;float ar=ch==0?0:cw/(float)ch;float ratio=area/(float)(dw*dh);
        ...
        int pxPad=Math.max(8,Math.round(bw*.06f)),pyPad=Math.max(8,Math.round(bh*.06f));
        int x0=Math.max(0,Math.round((bx-pxPad)/scale)),y0=Math.max(0,Math.round((by-pyPad)/scale));int x1=Math.min(sw,Math.round((bx+bw+pxPad)/scale)),y1=Math.min(sh,Math.round((by+bh+pyPad)/scale));
