...
public class MainActivityV2 extends FragmentActivity {
    ...
    private void expiries(){
        ...
        for(int target:targets){LinearLayout group=col();TextView head=tv("🔔 "+target+" DÍA"+(target==1?"":"S"),16,BLUE,true);group.addView(head);for(int i=0;i<a.length();i++){JSONObject x=a.optJSONObject(i);if(x==null)continue;long d=daysUntil(x.optString("expiry"));if(d==target){any=true;TextView row=tv(clientKey(x)+"\nDNI: "+x.optString("identityNumber","—")+" · Vence: "+x.optString("expiry","—"),15,TEXT,false);row.setBackground(box(Color.WHITE,14));group.addView(row,new LinearLayout.LayoutParams(-1,dp(68)));}}body.addView(group);}
    ...
    private void addClientCard(LinearLayout list,JSONObject x){
        ...
        Button b=btn("👤  "+title+(sub.trim().isEmpty()?"":"\n"+sub),false);b.setGravity(Gravity.CENTER_VERTICAL|Gravity.LEFT);b.setOnClickListener(v->detail(x));list.addView(b,new LinearLayout.LayoutParams(-1,dp(76)));
    }
    ...
    private void addRead(LinearLayout p,String label,String value){TextView t=tv(label+"\n"+(value==null||value.isEmpty()?"—":value),15,TEXT,false);t.setBackground(box(Color.WHITE,12));p.addView(t,new LinearLayout.LayoutParams(-1,dp(62)));}
    ...
    private void policyDetail(JSONObject p){
        ...
        String insured=p.optString("insured","");l.addView(tv("ASEGURADOS",16,BLUE,true));if(insured.isEmpty())l.addView(tv("No hay asegurados asociados.",14,MUTED,false));else for(String n:insured.split("[,;\\n]+")){if(!n.trim().isEmpty()){Button b=btn("👤 "+n.trim(),false);l.addView(b,new LinearLayout.LayoutParams(-1,dp(50)));b.setOnClickListener(v->new AlertDialog.Builder(this).setTitle(n.trim()).setMessage("Datos del asegurado disponibles en la ficha del cliente/póliza.").setPositiveButton("Cerrar",null).show());}}
    ...
    private void policies(){
        ...
        for(JSONObject p:all){Button b=btn("▣ "+p.optString("type","Póliza")+" · "+p.optString("number","—")+"\n"+p.optString("_holder",""),false);b.setOnClickListener(v->policyDetail(p));body.addView(b,new LinearLayout.LayoutParams(-1,dp(72)));}
    ...
    private void ocrImage(){TextRecognizer r=TextRecognition.getClient(TextRecognizerOptions.DEFAULT_OPTIONS);r.process(InputImage.fromBitmap(currentBitmap,0)).addOnSuccessListener(t->{String text=t.getText()==null?"":t.getText();if(side==2)backText=text;else frontText=text;fillFields(parseEssential(frontText+"\n"+backText));r.close();}).addOnFailureListener(e->{r.close();Toast.makeText(this,"OCR: "+e.getMessage(),Toast.LENGTH_LONG).show();});}
    ...
    private void saveOcrAsClient(){try{JSONObject x=new JSONObject();x.put("name",nameE.getText().toString().trim());x.put("surname",surnameE.getText().toString().trim());x.put("holder",(nameE.getText().toString()+" "+surnameE.getText().toString()).trim());x.put("identityNumber",dniE.getText().toString().trim());x.put("birthDate",birthE.getText().toString().trim());x.put("expiry",expiryE.getText().toString().trim());x.put("ocrText",frontText+"\n"+backText);x.put("image",currentImagePath);x.put("policies",new JSONArray());upsertClient(x);Toast.makeText(this,"Cliente guardado correctamente.",Toast.LENGTH_LONG).show();clients();}catch(Exception e){Toast.makeText(this,"No se pudo guardar: "+e.getMessage(),Toast.LENGTH_LONG).show();}}

    private JSONObject parseEssential(String raw){
        JSONObject x=new JSONObject();try{
            String u=normalize(raw);String[] lines=u.split("\\n");String surname=multiLabel(lines);String name=singleLabel(lines,"NOMBRE");if(name.isEmpty())name=mrzName(raw,true);if(surname.isEmpty())surname=mrzName(raw,false);String dni=findId(u);String birth=findBirth(u);String exp=findAfter(u,"VALIDEZ","CADUCIDAD","VÁLIDEZ");if(exp.isEmpty())exp=mrzDates(raw)[1];if(birth.isEmpty())birth=mrzDates(raw)[0];if(!mrzName(raw,false).isEmpty())surname=mrzName(raw,false);if(!mrzName(raw,true).isEmpty())name=mrzName(raw,true);if(dni.isEmpty())dni=findId(raw.toUpperCase(Locale.ROOT).replaceAll("\\s+",""));
    ...
    private String normalize(String s){return s.toUpperCase(Locale.ROOT).replace('\r','\n').replace("APELLlDOS","APELLIDOS").replace("NACIMlENTO","NACIMIENTO").replace("VALlDEZ","VALIDEZ").replaceAll("[ \\t]+"," ");}
    ...
    private String mrzName(String raw,boolean first){String s=raw.toUpperCase(Locale.ROOT).replaceAll("[\\r\\n ]+","");Matcher m=Pattern.compile("([A-ZÁÉÍÓÚÑ]+(?:<[A-ZÁÉÍÓÚÑ]+)*)<<([A-ZÁÉÍÓÚÑ]+(?:<[A-ZÁÉÍÓÚÑ]+)*)").matcher(s);if(m.find())return clean((first?m.group(2):m.group(1)).replace('<',' '));return "";}
    ...
    private void upsertClient(JSONObject x){JSONArray a=clientsData();String id=x.optString("identityNumber","");boolean replaced=false;for(int i=0;i<a.length();i++){JSONObject old=a.optJSONObject(i);if(old!=null&&(!id.isEmpty()&&id.equalsIgnoreCase(old.optString("identityNumber","")))){a.put(i,x);replaced=true;break;}}if(!replaced)a.put(x);prefs.edit().putString("rgapro_clients",a.toString()).apply();}
