...
public final class DocumentClassifier {
    ...
    public static Result classify(String raw){
        ...
        // El reverso DNI/NIE tiene prioridad y nunca se envía al parser de pólizas.
    ...
    private static int frontScore(String u){
        ...
        if(u.contains("APELLIDOS"))s+=5;if(u.contains("NOMBRE"))s+=4;if(u.contains("NACIONALIDAD"))s+=3;
        if(u.contains("NACIMIENTO"))s+=3;if(u.contains("SEXO"))s+=2;if(u.contains("ESPANA"))s+=2;if(DNI.matcher(u).find())s+=5;
        return s;
    }
    private static int backScore(String u){
        int s=0;if(u.contains("IDESP"))s+=10;if(u.contains("DOMICILIO"))s+=5;if(u.contains("LUGAR DE NACIMIENTO"))s+=4;
        if(u.contains("HIJO DE"))s+=3;if(u.contains("EQUIPO"))s+=2;if(MRZ.matcher(u).find()||u.contains("<<<<"))s+=8;return s;
    }
    private static int policyScore(String u){
        int s=0;if(u.contains("POLIZA")||u.contains("POLIZA DE SEGURO")||u.contains("CONDICIONES PARTICULARES"))s+=5;
        if(u.contains("TOMADOR")||u.contains("ASEGURADO"))s+=3;if(u.contains("FECHA EFECTO")||u.contains("EFECTO"))s+=2;
        if(u.contains("PRIMA")||u.contains("CAPITAL ASEGURADO")||u.contains("RECIBO"))s+=2;if(u.contains("OCASO")||u.contains("AEGON"))s+=2;
        if(DATE.matcher(u).find())s+=1;return s;
    }
    private static int receiptScore(String u){
        int s=0;if(u.contains("RECIBO"))s+=5;if(u.contains("IMPORTE")||u.contains("TOTAL A PAGAR")||u.contains("TOTAL"))s+=3;
        if(u.contains("VENCIMIENTO")||u.contains("FECHA DE CARGO"))s+=2;if(u.contains("IBAN")||u.contains("DOMICILIACION"))s+=2;return s;
    }
    private static String norm(String s){
        String x=s==null?"":s.toUpperCase(Locale.ROOT);x=Normalizer.normalize(x,Normalizer.Form.NFD).replaceAll("\\p{M}","");
