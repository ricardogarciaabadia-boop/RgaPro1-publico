...
identity=r'''    private static String extractIdentity(String text,String[] lines){
        ...
        for(int i=b[0];i<b[1];i++){ArrayList<String> vals=identityValues(lines[i]);if(!vals.isEmpty())for(String v:vals)if(isValidIdentity(v))return v;}
        return "";
    }
'''
...
# Add structured Hogar dates/postal code without relying on generic OCR labels.
...
# Extend parse() after parser patches have run.
...
# Patch the review/save UI in the same CI step so the workflow does not need another step.
...
if old not in m: raise SystemExit('review UI anchor not found')
...
if old not in m: raise SystemExit('helper anchor not found')
m=m.replace(old,new,1)
...
m=m.replace(old,new,1)
# Add the missing field variables if not already present.
...
print('policy parser v5 + policy review fields applied')
