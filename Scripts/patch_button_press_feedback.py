...
old='''    private Button btn(String text,boolean primary){Button b=new Button(this);b.setText(text);b.setTextSize(15);b.setAllCaps(false);b.setTextColor(primary?Color.WHITE:TEXT);b.setBackground(box(primary?BLUE:Color.WHITE,18));b.setPadding(dp(10),dp(6),dp(10),dp(6));}'''
# The current source has the previously patched multiline method, so replace it by locating the method structurally.
...
new='''    private Button btn(String text,boolean primary){
        ...
        b.setOnTouchListener((v,event)->{if(event.getAction()==MotionEvent.ACTION_UP){b.setBackground(box(Color.RED,18));b.setTextColor(Color.WHITE);}return false;});
        return b;
    }'''
s=s[:start]+new+s[end:]
P.write_text(s,encoding='utf-8');print('button feedback v2 applied')